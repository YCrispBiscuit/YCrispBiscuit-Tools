<template>
    ...
    <div class="Docus-body">
        ...
        <div class="doc-list-wrapper" ref="docListRef">
            <n-card v-for="(item, idx) in docList" :key="item.key" hoverable class="doc-big-card" :class="{ 'slide-in': itemVisible[idx] }">
...
<style scoped>
.Docus-body {
    position: relative; /* 让伪元素绝对定位于此容器内 */
...
.Docus-body::before {
    ...
    inset: 0; /* 顶部、右侧、底部、左侧都为0，撑满父容器 */
    background-image: url('@/assets/DocuBack.jpg'); /* 设置背景图片，与原背景一致 */
    background-size: cover; /* 背景图片铺满容器 */
    background-position: center; /* 背景图片居中显示 */
    background-attachment: fixed; /* 背景图片固定，不随内容滚动 */
    filter: blur(8px); /* 应用8px的高斯模糊效果 */
    opacity: 0.5; /* 设置不透明度为0.5，实现半透明 */
    z-index: 0; /* 保证伪元素在内容下方 */
    pointer-events: none; /* 禁止鼠标事件穿透，避免影响内容交互 */
}

.Docus-body > * {
    position: relative; /* 让内容层级高于伪元素 */
