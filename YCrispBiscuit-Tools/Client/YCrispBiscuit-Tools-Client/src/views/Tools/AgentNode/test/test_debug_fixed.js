...
console.log('修复后字符串的697位置附近:');
console.log('位置690-710:', JSON.stringify(fixed.slice(690, 710)));
console.log('位置695-700:', JSON.stringify(fixed.slice(695, 700)));

// 查找source_code字段
const sourceCodePattern = /"source_code":\s*"([^"]*(?:\\.[^"]*)*)"/g;
let match;
let index = 0;
console.log('\n检查source_code字段...');
while ((match = sourceCodePattern.exec(fixed)) !== null) {
    index++;
    console.log(`source_code #${index}:`);
    console.log('- 开始位置:', match.index);
    console.log('- 结束位置:', match.index + match[0].length);
    console.log('- 内容长度:', match[1].length);
    console.log('- 内容片段:', match[1].substring(0, 50) + '...');
}

// 尝试一个简化版本的字符串清理
function simpleCleanJson(str) {
    return str
        .replace(/"source_code":\s*"[^"]*"/g, '"source_code": "清理的代码内容"')
        .replace(/\n/g, '\\n')
        .replace(/\r/g, '\\r')
        .replace(/\t/g, '\\t');
}
...
console.log('\n尝试简化清理方法...');
try {
    const simplified = simpleCleanJson(problemJson);
